...
pub fn json_decode_to_dynamic_decode_error(
  ...
  case json_error {
    ...
    UnexpectedByte(byte, position) -> [
      dynamic.DecodeError(expected: "other byte", found: byte, path: [
...
pub fn json_decode_error(reason: String) -> GlomeError {
  JsonDecodeError(
    string.concat(["could not decode json due to: [ ", reason, " ]"]),
  )
}
...
pub fn map_decode_errors(
  ...
  result.map_error(errors, fn(decode_errors: DecodeErrors) {
...
pub fn map_decode_error(error: Result(a, DecodeError)) -> Result(a, GlomeError) {
  result.map_error(error, fn(decode_error: DecodeError) {
...
pub fn map_connection_error(
  ...
  result.map_error(error, fn(conn_error: ConnectError) {
    case conn_error {
